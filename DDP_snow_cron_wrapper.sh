#!/usr/bin/env bash
# DDP_snow_cron_wrapper.sh
# Released under the MIT license. Copyright (c) 2026, Vlad von Grigorian <>
#
# Cron-safe wrapper for the DDP playbook (DDP_drop_pluggable_database.yml).
# ServiceNow-driven Oracle pluggable database decommission (drop).
#
...
#
# This is the ServiceNow counterpart of DDP (no Jira counterpart -- ServiceNow
# only). It is structurally identical to PDB_snow_cron_wrapper.sh -- only the
# playbook, lock/log paths, and log labels differ -- so all SNOW project
# wrappers can coexist in /etc/ansible with separate lock files and log
...
# proceeds normally.
#
# NOTE (DDP is destructive): unlike PDB create, every DDP run that reaches
# Implement can DROP a pluggable database including datafiles. The playbook's
# own guardrails (mandatory approval via the normal change type, a literal
# "CONFIRM DROP <PDB>" description line, and a protected-PDB blocklist) are
# what keep this safe to run unattended -- this wrapper does not add any
# additional gating of its own.
#
# Crontab entry (every 5 minutes), as ansible_admin:
#   */5 * * * * /etc/ansible/DDP_snow_cron_wrapper.sh >> /tmp/cron.DDP.snow.log 2>&1
...
PLAYBOOK="DDP_drop_pluggable_database.yml"
VAULT_PASS_FILE="/etc/ansible/.vault_pass"     # file containing the vault password

BASE_DIR="${HOME}/snow_ddp"                  # user-owned; survives reboots
LOCK_FILE="${BASE_DIR}/snow_ddp.pid"
...
log "========================================================"
log "ServiceNow DDP wrapper started (PID $$)"
