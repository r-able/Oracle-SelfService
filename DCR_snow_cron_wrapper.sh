#!/usr/bin/env bash
# DCR_snow_cron_wrapper.sh
# Released under the MIT license. Copyright (c) 2026, Vlad von Grigorian <
#
# Cron-safe wrapper for the DCR playbook (SNOW_DCR_data_change.yml).
# ServiceNow-driven Oracle data change (code migration).
#
...
#
# This is the ServiceNow counterpart of DCR_jira_cron_wrapper.sh. It is
...
#   */5 * * * * /etc/ansible/DCR_snow_cron_wrapper.sh >> /tmp/cron.DCR.snow.log 2>&1
...
PLAYBOOK="SNOW_DCR_data_change.yml"
VAULT_PASS_FILE="/etc/ansible/.vault_pass"     # file containing the vault password

BASE_DIR="${HOME}/snow_dcr"                  # user-owned; survives reboots
LOCK_FILE="${BASE_DIR}/snow_dcr.pid"
...
log "========================================================"
log "ServiceNow DCR wrapper started (PID $$)"
