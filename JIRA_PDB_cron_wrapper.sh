...
# JIRA_PDB_cron_wrapper.sh
#
# Cron-safe wrapper for the PDB playbook (JIRA_PDB_create_pluggable_database.yml).
#
...
#   */5 * * * * /etc/ansible/JIRA_PDB_cron_wrapper.sh >> /tmp/cron.PDB.log 2>&1
...
PLAYBOOK="JIRA_PDB_create_pluggable_database.yml"
VAULT_PASS_FILE="/etc/ansible/.vault_pass"     # file containing the vault password

BASE_DIR="${HOME}/jira_pdb"                    # user-owned; survives reboots
LOCK_FILE="${BASE_DIR}/jira_pdb.pid"
...
log "========================================================"
log "Jira PDB wrapper started (PID $$)"
