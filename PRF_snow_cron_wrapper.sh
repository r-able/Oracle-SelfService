#!/usr/bin/env bash
# PRF_snow_cron_wrapper.sh
# Released under the MIT license. Copyright (c) 2026, Vlad von Grigorian <>
#
# Cron-safe wrapper for the PRF playbook (PRF_snow_performance_analysis.yml).
# ServiceNow-driven Oracle AWR performance analysis + SQL Tuning Advisor.
#
...
#
# This is the ServiceNow counterpart of PRF (no Jira counterpart - ServiceNow
# only, built new for this project). It is structurally identical to
# PDB_snow_cron_wrapper.sh / DDP_snow_cron_wrapper.sh - only the playbook,
# lock/log paths, and log labels differ - so all SNOW project wrappers can
# coexist in /etc/ansible with separate lock files and log directories and
# never collide.
#
# Lock and log paths live under the invoking user's home directory. Those
# system trees (/var/run, /var/log) are root-owned (and /run is a tmpfs
# recreated empty at every boot), so an unprivileged cron user can never rely
# on them without root-side setup that a rebuild silently loses. Everything
# this script touches is self-created and user-owned.
#
...
#
# NOTE (ServiceNow PDI hibernation): a Personal Developer Instance hibernates
# after ~30 minutes idle. When it is asleep the playbook's ServiceNow query
# fails fast and this wrapper logs a non-zero rc - that is expected, not a
# controller fault. Wake the instance in a browser and the next cron slot
# proceeds normally.
#
# NOTE (PRF is read-only analysis, no approval): unlike DDP, PRF's own
# change request never touches the database - execute_tuning_task only
# produces advice. The SEPARATE implementation follow-up change PRF opens
# on success DOES require approval before a human DBA acts on it, so this
# wrapper is safe to run unattended on a tight schedule.
#
# NOTE (PRF can be long-running): AWR report generation, the top-5 SQL_ID
# query, and up to 5 SQL Tuning Advisor runs (each with its own time_limit)
# happen in a single pass. Keep the cron interval wide enough that a slow
# tuning-advisor pass on a busy database does not overlap the next slot -
# the PID lock handles this safely either way, but a shorter interval just
# means more skipped "still active" log lines.
#
# Crontab entry (every 5 minutes), as ansible_admin:
#   */5 * * * * /etc/ansible/PRF_snow_cron_wrapper.sh >> /tmp/cron.PRF.snow.log 2>&1
...
LOCK_FILE="${BASE_DIR}/snow_prf.pid"
...
log "========================================================"
log "ServiceNow PRF wrapper started (PID $$)"
