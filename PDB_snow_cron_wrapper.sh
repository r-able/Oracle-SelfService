#!/usr/bin/env bash
# PDB_snow_cron_wrapper.sh
# Released under the MIT license. Copyright (c) 2026, Vlad von Grigorian <
#
# Cron-safe wrapper for the PDB playbook (SNOW_PDB_create_pluggable_database.yml).
# ServiceNow-driven Oracle pluggable database creation.
#
...
#
# This is the ServiceNow counterpart of PDB (no Jira counterpart -- ServiceNow only). It is
# structurally identical -- only the playbook, lock/log paths, and log labels
# differ -- so the Jira and ServiceNow suites can coexist in /etc/ansible with
# separate lock files and log directories and never collide.
#
...
#     recreated empty at every boot), so an unprivileged cron user can never
#     rely on them without root-side setup that a rebuild silently loses.
#     Everything this script touches is self-created and user-owned.
...
# after ~30 minutes idle. When it is asleep the playbook's pre-flight fails
# fast with a "wake it at developer.servicenow.com" message and this wrapper
# logs a non-zero rc -- that is expected, not a controller fault. Wake the
# instance in a browser and the next cron slot proceeds normally.
#
# Crontab entry (every 5 minutes), as ansible_admin:
#   */5 * * * * /etc/ansible/PDB_snow_cron_wrapper.sh >> /tmp/cron.PDB.snow.log 2>&1
...
PLAYBOOK="SNOW_PDB_create_pluggable_database.yml"
VAULT_PASS_FILE="/etc/ansible/.vault_pass"     # file containing the vault password

BASE_DIR="${HOME}/snow_pdb"                  # user-owned; survives reboots
LOCK_FILE="${BASE_DIR}/snow_pdb.pid"
...
log "========================================================"
log "ServiceNow PDB wrapper started (PID $$)"
