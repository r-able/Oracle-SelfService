...
- **Ollama path (default, $0):** install Ollama on whichever box runs
  ...
  model: `ollama pull llama3.1:8b` (or whatever `ollama_model` is
  currently set to in `demo_vars.yml` — keep this in sync if that value
  ever changes again). Confirm it's reachable at `ollama_host` before the
  first real run.
