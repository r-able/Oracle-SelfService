#!/usr/bin/env bash
# JIRA_DDP_cron_wrapper.sh
# Released under the MIT license. Copyright (c) 2026, Vlad von Grigorian <
#
# Cron-safe wrapper for the DDP playbook (JIRA_DDP_drop_pluggable_database.yml).
#
...
#
# NOTE: this playbook performs an IRREVERSIBLE, DESTRUCTIVE action (dropping
# a pluggable database). The playbook's own safety gates (protected-PDB
# list, required confirmation string, pre-drop snapshot) are the actual
# safety net - this wrapper is only about safe unattended execution
# (locking, logging), not about the safety of the operation itself.
#
# Crontab entry (every 1 minute, matching every other JIRA project's
# cadence), as ansible_admin:
...
PLAYBOOK="JIRA_DDP_drop_pluggable_database.yml"
VAULT_PASS_FILE="/etc/ansible/.vault_pass"     # file containing the vault password

BASE_DIR="${HOME}/jira_ddp"                    # user-owned; survives reboots
LOCK_FILE="${BASE_DIR}/jira_ddp.pid"
...
log "========================================================"
log "Jira DDP wrapper started (PID $$)"
...
# ── Log rotation ──────────────────────────────────────────────────────────────
# A 1-minute cron produces many log files/day; prune anything older than the
