#!/usr/bin/env bash
# JIRA_sync_oracle_schemas_cron_wrapper.sh
# Released under the MIT license. Copyright (c) 2026, Vlad von Grigorian <>
#
# Cron-safe wrapper for JIRA_sync_oracle_schemas.yml - keeps REF's
# "Source Schema" / "Target Schema" field options in sync with the real,
# non-system schemas found in every known host:PDB, so REF's rebuilt
# cross-host refresh form only ever offers schemas that genuinely exist.
#
...
#
# Suggested schedule: every 15 minutes, matching the cadence of the
# ServiceNow-side equivalent (SNOW_sync_oracle_schemas_cron_wrapper.sh, also
# */15) - schema discovery does two SQL round-trips per host:PDB rather
# than one, so a slightly less aggressive cadence than the 1-minute target
# sync keeps the fleet-wide load down without meaningfully delaying when a
# new schema shows up in the dropdown.
#
...
PLAYBOOK="JIRA_sync_oracle_schemas.yml"
VAULT_PASS_FILE="/etc/ansible/.vault_pass"

BASE_DIR="${HOME}/jira_sync_oracle_schemas"
LOCK_FILE="${BASE_DIR}/jira_sync_oracle_schemas.pid"
...
log "========================================================"
log "Jira oracle_schemas sync wrapper started (PID $$)"
